...
Usage: bash ipda_BRAIN_step011-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_BRAIN_step011-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_BRAIN_step011-to-pbs_${thislogdate}.txt
...
outpath_BRAIN011_FastQC="BRAIN_step011_qc-raw_FastQC_${thislogdate}"

## Create output directories
mkdir -p ${outpath_BRAIN011_FastQC}
...
date
echo "## Executing bash ipda_BRAIN_step011-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${outpath_BRAIN011_FastQC}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1="${path_file}_*1.f*"; f2="${path_file}_*2.f*"; echo "fastqc -t ${ncpus} --outdir ${outpath_BRAIN011_FastQC} ${f1} ${f2} --memory 10000" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including BRAIN step 011 jobs) at
qstat -u "$user"
...
sed -i 's,${outpath_BRAIN011_FastQC},'"${outpath_BRAIN011_FastQC}"',g' "$logfile"
