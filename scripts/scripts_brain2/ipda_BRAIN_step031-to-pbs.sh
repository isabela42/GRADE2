...
Usage: bash ipda_BRAIN_step031-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"

Resources baseline: -m 10 -c 1 -w "02:00:00"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col2:
                            path/from/working/dir/to/BRAIN_step021_trim-reads-adapters_Trimmomatic_DATE/adapter-trimmed_stem_R1.f* (line 1)
                            path/from/working/dir/to/BRAIN_step021_trim-reads-adapters_Trimmomatic_DATE/adapter-trimmed_stem_R2.f* (line 2)
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_BRAIN_step031-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_BRAIN_step031-to-pbs_${thislogdate}.txt
...
outpath_BRAIN031_FastQC="BRAIN_step031_qc-trimm_FastQC_${thislogdate}"

## Create output directories
mkdir -p ${outpath_BRAIN031_FastQC}
...
date
echo "## Executing bash ipda_BRAIN_step031-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${outpath_BRAIN031_FastQC}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1="${path_file}_R1.f*"; f2="${path_file}_R2.f*"; echo "fastqc -t ${ncpus} --outdir ${outpath_BRAIN031_FastQC} ${f1} ${f2} --memory 10000" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including BRAIN step 01 jobs) at
qstat -u "$user"
...
sed -i 's,${module_fastqc},'"${module_fastqc}"',g' "$logfile"
sed -i 's,${outpath_BRAIN031_FastQC},'"${outpath_BRAIN031_FastQC}"',g' "$logfile"
