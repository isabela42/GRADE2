...
Usage: bash ipda_BRAIN_step091-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/BRAIN_step081_quantification_RSEM_DATE/
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_BRAIN_step091-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_BRAIN_step091-to-pbs_${thislogdate}.txt
...
date
echo "## Executing bash ipda_BRAIN_step091-to-pbs.sh"
...
date ## Status of all user jobs (including BRAIN step 091 jobs) at
qstat -u "$user"
