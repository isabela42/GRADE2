...
Usage: bash ipda_BRAIN_step032-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"

Resources baseline: -m 1 -c 1 -w "01:00:00"

## Input:

-i <path/to/input/folder>   path/from/working/dir/to/BRAIN_step031_QC-trimmed-files_FastQC_DATE/
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_BRAIN_step032-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_BRAIN_step032-to-pbs_${thislogdate}.txt
...
outpath_BRAIN032_MultiQC="BRAIN_step032_qc-trimm_MultiQC_${thislogdate}"

## Create output directories
mkdir -p ${outpath_BRAIN032_MultiQC}
...
date
echo "## Executing bash ipda_BRAIN_step032-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${outpath_BRAIN032_MultiQC}"
...
echo "${multiqc} ${input} -o ${outpath_BRAIN032_MultiQC}" >> ${pbs_stem}_${thislogdate}.pbs
...
date ## Status of all user jobs (includingBRAIN step 032 jobs) at
qstat -u "$user"
...
sed -i 's,${outpath_BRAIN032_MultiQC},'"${outpath_BRAIN032_MultiQC}"',g' "$logfile"
