...
Usage: bash ipda_BRAIN_step072-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"
...
80% of the mem and CPUS specified when running this script.
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/BRAIN_step071_alignment_SAMtools_DATE/stem.samtools.bam
                            full file paths
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_BRAIN_step072-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_BRAIN_step072-to-pbs_${thislogdate}.txt
...
# WARNING: Novosort uses more memory and CPUS than what is required.
# Because of it, we only ask for 60% of the resources
# specified when running this script, so that PBS jobs
# do not go over what was required.
memless=$((mem*80/100))
cpuless=$((ncpus*80/100))
...
outpath_BRAIN072_Novosort="BRAIN_step072_alignment_Novosort_${thislogdate}"

## Create output directories
mkdir -p ${outpath_BRAIN072_Novosort}
...
date
echo "## Executing bash ipda_BRAIN_step072-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${outpath_BRAIN072_Novosort}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "${path_file}" | cut -d"." -f1 | rev | cut -d"/" -f1 | rev` ; mkdir -p ${outpath_BRAIN072_Novosort}; echo "novosort -n -m ${memless}G -c ${cpuless} ${path_file} > ${outpath_BRAIN072_Novosort}/${file}.novosort.bam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including BRAIN step 072 jobs) at
qstat -u "$user"
...
sed -i 's,${outpath_BRAIN072_Novosort},'"${outpath_BRAIN072_Novosort}"',g' "$logfile"
