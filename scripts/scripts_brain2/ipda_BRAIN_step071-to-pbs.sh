...
Usage: bash ipda_BRAIN_step071-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/BRAIN_step061_alignment_STAR_DATE/stem
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_BRAIN_step071-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_BRAIN_step071-to-pbs_${thislogdate}.txt
...
outpath_BRAIN071_SAMtools="BRAIN_step071_alignment_SAMtools_${thislogdate}"

## Create output directories
mkdir -p ${outpath_BRAIN071_SAMtools}
...
date
echo "## Executing bash ipda_BRAIN_step071-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${outpath_BRAIN071_SAMtools}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "${path_file}" | rev | cut -d"/" -f1 | rev | cut -d"_" -f2-` ; echo "#................................................" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "${path_file}" | rev | cut -d"/" -f1 | rev | cut -d"_" -f2-` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "${path_file}" | rev | cut -d"/" -f1 | rev | cut -d"_" -f2-` ; echo 'echo "## Load tools from HPC"' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "${path_file}" | rev | cut -d"/" -f1 | rev | cut -d"_" -f2-` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "${path_file}" | rev | cut -d"/" -f1 | rev | cut -d"_" -f2-` ; echo 'echo "## Path to user-installed tools"' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "${path_file}" | rev | cut -d"/" -f1 | rev | cut -d"_" -f2-` ; echo 'echo "# None required"' >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "${path_file}" | rev | cut -d"/" -f1 | rev | cut -d"_" -f2-` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "${path_file}" | rev | cut -d"/" -f1 | rev | cut -d"_" -f2-` ; common=`echo "${path_file}" | rev | cut -d"/" -f1 | rev | cut -d"_" -f1` ; mkdir -p ${outpath_BRAIN071_SAMtools}; echo "samtools view -@ ${ncpus} ${path_file}/${common}_${file}Aligned.toTranscriptome.out.bam -f 3 -b >> ${outpath_BRAIN071_SAMtools}/${file}.samtools.bam" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including BRAIN step 071 jobs) at
qstat -u "$user"
...
sed -i 's,${outpath_BRAIN071_SAMtools},'"${outpath_BRAIN071_SAMtools}"',g' "$logfile"
