...
Usage: bash ipda_BRAIN_step081-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"
...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col1:
                            path/from/working/dir/to/BRAIN_step072_alignment_Novosort_DATE/stem.novosort.bam
                            full file paths
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_BRAIN_step081-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_BRAIN_step081-to-pbs_${thislogdate}.txt
...
outpath_BRAIN081_RSEM="BRAIN_step081_quantification_RSEM_${thislogdate}"

## Create output directories
mkdir -p ${outpath_BRAIN081_RSEM}
...
date
echo "## Executing bash ipda_BRAIN_step081-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${outpath_BRAIN081_RSEM}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "${path_file}" | cut -d"." -f1 | rev | cut -d"/" -f1 | rev` ; index=`grep -P "${path_file}\t" ${input} | cut -f2`; indexstem=`echo ${index} | rev | cut -d"/" -f1 | rev` ; mkdir -p ${outpath_BRAIN081_RSEM}; echo "rsem-calculate-expression --paired-end --bam --forward-prob 0 --no-bam-output -p ${ncpus} ${path_file} ${index} ${outpath_BRAIN081_RSEM}/rquant-${indexstem}_${file}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including BRAIN step 081 jobs) at
qstat -u "$user"
...
sed -i 's,${outpath_BRAIN081_RSEM},'"${outpath_BRAIN081_RSEM}"',g' "$logfile"
