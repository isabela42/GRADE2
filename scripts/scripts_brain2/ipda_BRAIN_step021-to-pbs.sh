...
Usage: bash ipda_BRAIN_step021-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_BRAIN_step021-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_BRAIN_step021-to-pbs_${thislogdate}.txt
...
outpath_BRAIN021_Trimmomatic="BRAIN_step021_trim-adapters_Trimmomatic_${thislogdate}"

## Create output directories
mkdir -p ${outpath_BRAIN021_Trimmomatic}
...
echo
echo "## Output files saved to:       ${outpath_BRAIN021_Trimmomatic}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1="${path_file}_*1.f*"; f2="${path_file}_*2.f*"; adapters_fasta=`grep ${path_file} ${input} | cut -f3 | sort | uniq`; echo "trimmomatic PE -phred33 -threads ${ncpus} ${f1} ${f2} ${outpath_BRAIN021_Trimmomatic}/adapter-trimmed_${file}_R1.fq.gz ${outpath_BRAIN021_Trimmomatic}/unpaired_adapter-trimmed_${file}_R1.fq.gz ${outpath_BRAIN021_Trimmomatic}/adapter-trimmed_${file}_R2.fq.gz ${outpath_BRAIN021_Trimmomatic}/unpaired_adapter-trimmed_${file}_R2.fq.gz ILLUMINACLIP:${adapters_fasta}:2:30:10 SLIDINGWINDOW:4:20 MINLEN:75 HEADCROP:12" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
sed -i 's,${outpath_BRAIN021_Trimmomatic},'"${outpath_BRAIN021_Trimmomatic}"',g' "$logfile"
