...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g' | cut -d'_' -f2-` ; f1=`ls ${path_file}_R1.f*`; f2=`ls ${path_file}_R2.f*`; index=`grep -P "${path_file}\t" ${input} | cut -f2`; indexstem=`echo ${index} | rev | cut -d"/" -f1 | rev`; gtf=`grep -P "${path_file}\t" ${input} | cut -f3`; mkdir -p ${outpath_GRADE2051_STAR}/salign-${indexstem}_${file}; echo "STAR --runMode alignReads --readFilesIn ${f1} ${f2} --readFilesCommand zcat --outFileNamePrefix ${outpath_GRADE2051_STAR}/salign-${indexstem}_${file}/salign-${indexstem}_${file} --genomeDir ${index} --sjdbGTFfile ${gtf} --outSJfilterReads Unique --sjdbOverhang 100 --runThreadN ${ncpus} --genomeLoad NoSharedMemory --outFilterType BySJout --outFilterMultimapNmax 1 --outFilterMismatchNmax 999 --outFilterMismatchNoverLmax 0.1 --alignSJoverhangMin 8 --alignSJDBoverhangMin 3 --alignIntronMin 20 --chimSegmentMin 20 --outSAMattributes All --outSAMstrandField intronMotif --quantMode TranscriptomeSAM --outSAMtype BAM SortedByCoordinate" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
