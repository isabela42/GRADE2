...
cut -f1 ${input} | sort | uniq | while read plot; do echo "    axis.title = element_text(size = 18)," >> ${pbs_stem}_${plot}_${thislogdate}.r; done
cut -f1 ${input} | sort | uniq | while read plot; do echo "    axis.text = element_text(size = 16)) +" >> ${pbs_stem}_${plot}_${thislogdate}.r; done
