...
Resources baseline: -m 10 -c 1 -w "30:00:00" #max recources usage with over 7k samples to compile
...
echo
echo "## Output files saved to:       ${outpath_GRADE2055_Bash}"
...
cut -f3 ${input} | sort | uniq | while read exp; do echo "cat ${outpath_GRADE2055_Bash}/${exp}-out-expcnt.tmp ${outpath_GRADE2055_Bash}/${exp}-genes-expcnt.tmp > ${outpath_GRADE2055_Bash}/RSEM-${exp}_genes-expcnt.tsv && rm -f ${outpath_GRADE2055_Bash}/${exp}-out-expcnt.tmp ${outpath_GRADE2055_Bash}/${exp}-genes-expcnt.tmp ${outpath_GRADE2055_Bash}/${exp}-tmp-expcnt_* " >> ${pbs_stem}_${exp}_${thislogdate}.pbs ; done
cut -f3 ${input} | sort | uniq | while read exp; do echo "" >> ${pbs_stem}_${exp}_${thislogdate}.pbs ; done
...
cut -f3 ${input} | sort | uniq | while read exp; do echo "cat ${outpath_GRADE2055_Bash}/${exp}-out-tpm.tmp ${outpath_GRADE2055_Bash}/${exp}-genes-tpm.tmp > ${outpath_GRADE2055_Bash}/RSEM-${exp}_genes-tpm.tsv && rm -f ${outpath_GRADE2055_Bash}/${exp}-out-tpm.tmp ${outpath_GRADE2055_Bash}/${exp}-genes-tpm.tmp ${outpath_GRADE2055_Bash}/${exp}-tmp-tpm_* " >> ${pbs_stem}_${exp}_${thislogdate}.pbs ; done
cut -f3 ${input} | sort | uniq | while read exp; do echo "" >> ${pbs_stem}_${exp}_${thislogdate}.pbs ; done
...
cut -f3 ${input} | sort | uniq | while read exp; do echo "cat ${outpath_GRADE2055_Bash}/${exp}-out-fpkm.tmp ${outpath_GRADE2055_Bash}/${exp}-genes-fpkm.tmp > ${outpath_GRADE2055_Bash}/RSEM-${exp}_genes-fpkm.tsv && rm -f ${outpath_GRADE2055_Bash}/${exp}-out-fpkm.tmp ${outpath_GRADE2055_Bash}/${exp}-genes-fpkm.tmp ${outpath_GRADE2055_Bash}/${exp}-tmp-fpkm_* " >> ${pbs_stem}_${exp}_${thislogdate}.pbs ; done
