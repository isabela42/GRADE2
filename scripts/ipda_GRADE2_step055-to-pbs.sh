...
cut -f3 ${input} | sort | uniq | while read exp; do echo "sort -k1 \$(grep \"${exp}\" ${input} | cut -f1 | head -n1) | cut -f1-2 > ${outpath_GRADE2055_Bash}/${exp}-transcripts-expcnt.tmp" >> ${pbs_stem}_${exp}_${thislogdate}.pbs ; done
cut -f3 ${input} | sort | uniq | while read exp; do echo "grep \"${exp}\" ${input} | while IFS=\$'\t' read -r filepath sampleid exp; do [[ -z \"\${filepath}\" || ! -f \"\${filepath}\" ]] && continue ; sort -k1 \${filepath} | cut -f5 > ${outpath_GRADE2055_Bash}/${exp}-tmp-expcnt_\${sampleid} ; done" >> ${pbs_stem}_${exp}_${thislogdate}.pbs ; done
...
cut -f3 ${input} | sort | uniq | while read exp; do echo "sort -k1 \$(grep \"${exp}\" ${input} | cut -f1 | head -n1) | cut -f1-2 > ${outpath_GRADE2055_Bash}/${exp}-transcripts-tpm.tmp" >> ${pbs_stem}_${exp}_${thislogdate}.pbs ; done
cut -f3 ${input} | sort | uniq | while read exp; do echo "grep \"${exp}\" ${input} | while IFS=\$'\t' read -r filepath sampleid exp; do [[ -z \"\${filepath}\" || ! -f \"\${filepath}\" ]] && continue ; sort -k1 \${filepath} | cut -f6 > ${outpath_GRADE2055_Bash}/${exp}-tmp-tpm_\${sampleid} ; done" >> ${pbs_stem}_${exp}_${thislogdate}.pbs ; done
...
cut -f3 ${input} | sort | uniq | while read exp; do echo "sort -k1 \$(grep \"${exp}\" ${input} | cut -f1 | head -n1) | cut -f1-2 > ${outpath_GRADE2055_Bash}/${exp}-transcripts-fpkm.tmp" >> ${pbs_stem}_${exp}_${thislogdate}.pbs ; done
cut -f3 ${input} | sort | uniq | while read exp; do echo "grep \"${exp}\" ${input} | while IFS=\$'\t' read -r filepath sampleid exp; do [[ -z \"\${filepath}\" || ! -f \"\${filepath}\" ]] && continue ; sort -k1 \${filepath} | cut -f7 > ${outpath_GRADE2055_Bash}/${exp}-tmp-fpkm_\${sampleid} ; done" >> ${pbs_stem}_${exp}_${thislogdate}.pbs ; done
