...
Usage: bash ipda_GRADE2_step011-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_GRADE2_step011-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_GRADE2_step011-to-pbs_${thislogdate}.txt
...
outpath_GRADE2011_FastQC="grade011_qc-raw_FastQC_${thislogdate}"

## Create output directories
mkdir -p ${outpath_GRADE2011_FastQC}
...
date
echo "## Executing bash ipda_GRADE2_step011-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${outpath_GRADE2011_FastQC}"
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; f1=`grep -F "${path_file}" ${input} | cut -f2 | head -n1`; f2=`grep -F "${path_file}" ${input} | cut -f2 | tail -n1`; echo "fastqc -t ${ncpus} --outdir ${outpath_GRADE2011_FastQC} ${f1} ${f2} --memory 10000" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
date ## Status of all user jobs (including GRADE2 step 011 jobs) at
qstat -u "$user"
...
sed -i 's,${outpath_GRADE2011_FastQC},'"${outpath_GRADE2011_FastQC}"',g' "$logfile"
