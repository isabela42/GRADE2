...
Usage: bash ipda_GRADE2_step003-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_GRADE2_step003-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_GRADE2_step003-to-pbs_${thislogdate}.txt
...
date
echo "## Executing bash ipda_GRADE2_step003-to-pbs.sh"
...
date ## Status of all user jobs (including GRADE2 step 003 jobs) at
qstat -u "$user"
