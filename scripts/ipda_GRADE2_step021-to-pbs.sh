...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; flag_merge=`grep -F "${path_file}" ${input} | cut -f4 | sort | uniq`; if [[ $flag_merge = "yes" ]]; then echo "zcat -c ${path_file}*_*1.f* | gzip -c >> ${outpath_GRADE2021_Trimmomatic}/${file}_R1.fastq.gz" >> ${pbs_stem}_${file}_${thislogdate}.pbs; elif [[ $flag_merge = "no" ]]; then echo "#Flag to do not merge file" >> ${pbs_stem}_${file}_${thislogdate}.pbs; elif [[ $flag_merge = "single" ]]; then echo "#Single reads flagged; not merging file" >> ${pbs_stem}_${file}_${thislogdate}.pbs; else echo "Unknown flag provided. Please provide either yes, no or single string and try again" >> ${pbs_stem}_${file}_${thislogdate}.pbs; fi; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; flag_merge=`grep -F "${path_file}" ${input} | cut -f4 | sort | uniq`; if [[ $flag_merge = "yes" ]]; then echo "zcat -c ${path_file}*_*2.f* | gzip -c >> ${outpath_GRADE2021_Trimmomatic}/${file}_R2.fastq.gz" >> ${pbs_stem}_${file}_${thislogdate}.pbs; elif [[ $flag_merge = "no" ]]; then echo "#Flag to do not merge file" >> ${pbs_stem}_${file}_${thislogdate}.pbs; elif [[ $flag_merge = "single" ]]; then echo "#Single reads flagged; not merging file" >> ${pbs_stem}_${file}_${thislogdate}.pbs; else echo "Unknown flag provided. Please provide either yes, no or single string and try again" >> ${pbs_stem}_${file}_${thislogdate}.pbs; fi; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
