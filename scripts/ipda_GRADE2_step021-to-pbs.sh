...
if [[ "$test" == true ]]; then cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "module load ${module_jobperf}" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done; fi
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; flag_merge=`grep -F "${path_file}" ${input} | cut -f4 | sort | uniq`; if [[ $flag_merge = "yes" ]]; then mergeF1=`grep -F "${path_file}" ${input} | cut -f2 | head -n1`; echo "zcat -c ${mergeF1} | gzip -c >> ${outpath_GRADE2021_Trimmomatic}/${file}_R1.fastq.gz" >> ${pbs_stem}_${file}_${thislogdate}.pbs; elif [[ $flag_merge = "no" ]]; then echo "#Flag to do not merge file" >> ${pbs_stem}_${file}_${thislogdate}.pbs; elif [[ $flag_merge = "single" ]]; then echo "#Single reads flagged; not merging file" >> ${pbs_stem}_${file}_${thislogdate}.pbs; else echo "Unknown flag provided. Please provide either yes, no or single string and try again" >> ${pbs_stem}_${file}_${thislogdate}.pbs; fi; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; flag_merge=`grep -F "${path_file}" ${input} | cut -f4 | sort | uniq`; if [[ $flag_merge = "yes" ]]; then mergeF2=`grep -F "${path_file}" ${input} | cut -f2 | tail -n1`; echo "zcat -c ${mergeF2} | gzip -c >> ${outpath_GRADE2021_Trimmomatic}/${file}_R2.fastq.gz" >> ${pbs_stem}_${file}_${thislogdate}.pbs; elif [[ $flag_merge = "no" ]]; then echo "#Flag to do not merge file" >> ${pbs_stem}_${file}_${thislogdate}.pbs; elif [[ $flag_merge = "single" ]]; then echo "#Single reads flagged; not merging file" >> ${pbs_stem}_${file}_${thislogdate}.pbs; else echo "Unknown flag provided. Please provide either yes, no or single string and try again" >> ${pbs_stem}_${file}_${thislogdate}.pbs; fi; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; flag_merge=`grep -F "${path_file}" ${input} | cut -f4 | sort | uniq`; adapters_fasta=`grep -F "${path_file}" ${input} | cut -f3 | sort | uniq`; if [[ $flag_merge = "yes" ]]; then f1="${outpath_GRADE2021_Trimmomatic}/${file}_R1.fastq.gz"; f2="${outpath_GRADE2021_Trimmomatic}/${file}_R2.fastq.gz";  echo "trimmomatic PE -phred33 -threads ${ncpus} ${f1} ${f2} ${outpath_GRADE2021_Trimmomatic}/adapter-trimmed_${file}_R1.fq.gz ${outpath_GRADE2021_Trimmomatic}/unpaired_adapter-trimmed_${file}_R1.fq.gz ${outpath_GRADE2021_Trimmomatic}/adapter-trimmed_${file}_R2.fq.gz ${outpath_GRADE2021_Trimmomatic}/unpaired_adapter-trimmed_${file}_R2.fq.gz ILLUMINACLIP:${adapters_fasta}:2:30:10 SLIDINGWINDOW:4:20 MINLEN:75 HEADCROP:12" >> ${pbs_stem}_${file}_${thislogdate}.pbs; elif [[ $flag_merge = "no" ]]; then f1=`grep -F "${path_file}" ${input} | cut -f2 | head -n1`; f2=`grep -F "${path_file}" ${input} | cut -f2 | tail -n1`; echo "trimmomatic PE -phred33 -threads ${ncpus} ${f1} ${f2} ${outpath_GRADE2021_Trimmomatic}/adapter-trimmed_${file}_R1.fq.gz ${outpath_GRADE2021_Trimmomatic}/unpaired_adapter-trimmed_${file}_R1.fq.gz ${outpath_GRADE2021_Trimmomatic}/adapter-trimmed_${file}_R2.fq.gz ${outpath_GRADE2021_Trimmomatic}/unpaired_adapter-trimmed_${file}_R2.fq.gz ILLUMINACLIP:${adapters_fasta}:2:30:10 SLIDINGWINDOW:4:20 MINLEN:75 HEADCROP:12" >> ${pbs_stem}_${file}_${thislogdate}.pbs; elif [[ $flag_merge = "single" ]]; then f1=`grep -F "${path_file}" ${input} | cut -f2 | head -n1`; echo "trimmomatic SE -phred33 -threads ${ncpus} ${f1} ${outpath_GRADE2021_Trimmomatic}/adapter-trimmed_${file}_R1.fq.gz ILLUMINACLIP:${adapters_fasta}:2:30:10 SLIDINGWINDOW:4:20 MINLEN:40 HEADCROP:12" >> ${pbs_stem}_${file}_${thislogdate}.pbs ; fi; done
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done

if [[ "$test" == true ]]; then cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; echo "date >&2" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done; fi
cut -f1 ${input} | sort | uniq | while read path_file; do file=`echo "$(basename "${path_file%%.*}" | sed 's/\(.*\)\..*/\1/')" | sed 's/\*//g'` ; flag_merge=`grep -F "${path_file}" ${input} | cut -f4 | sort | uniq`; echo "rm -f ${outpath_GRADE2021_Trimmomatic}/unpaired_*" >> ${pbs_stem}_${file}_${thislogdate}.pbs; done
