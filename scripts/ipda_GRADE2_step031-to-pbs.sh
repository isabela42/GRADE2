...
Usage: bash ipda_GRADE2_step031-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_GRADE2_step031-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
date
echo "## Executing bash ipda_GRADE2_step031-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${outpath_GRADE2031_FastQC}"
...
date ## Status of all user jobs (including GRADE2 step 01 jobs) at
qstat -u "$user"
...
sed -i 's,${outpath_GRADE2031_FastQC},'"${outpath_GRADE2031_FastQC}"',g' "$logfile"
