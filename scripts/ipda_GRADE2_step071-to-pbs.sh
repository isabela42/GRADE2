...
cut -f1 ${input} | sort | uniq | while read stem; do inputcnt=`grep "${stem}" ${input} | cut -f2 | sort | uniq`; inputanno=`grep "${stem}" ${input} | cut -f3 | sort | uniq`; groups=`grep "${stem}" ${input} | cut -f4 | sort | uniq`; colnums=`grep "${stem}" ${input} | cut -f5 | sort | uniq`; contrasts=`grep "${stem}" ${input} | cut -f6 | sort | uniq`; merge=`grep "${stem}" ${input} | cut -f7 | sort | uniq`; mergecontrasts=`grep "${stem}" ${input} | cut -f8 | sort | uniq`; genesets=`grep "${stem}" ${input} | cut -f9 | sort | uniq`; label=`grep "${stem}" ${input} | cut -f10 | sort | uniq`; inputgmt=`grep "${stem}" ${input} | cut -f11 | sort | uniq`; rscript=`grep "${stem}" ${input} | cut -f12 | sort | uniq`; rfunctions=`grep "${stem}" ${input} | cut -f13 | sort | uniq`; echo "Rscript ${rscript} --inputanno ${inputanno} --inputexp ${inputcnt}_isoforms-expcnt.tsv --inputtpm ${inputcnt}_isoforms-tpm.tsv --level \"transcript\" --groups ${groups} --colnums ${colnums} --contrasts ${contrasts} --merge ${merge} --mergecontrasts ${mergecontrasts} --genesets ${genesets} --plotlabel ${label} --inputgmt ${inputgmt} --outdir ${outpath_GRADE2071_plots}/ --outstem \"${stem}\" --function ${rfunctions}" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read stem; do inputcnt=`grep "${stem}" ${input} | cut -f2 | sort | uniq`; inputanno=`grep "${stem}" ${input} | cut -f3 | sort | uniq`; groups=`grep "${stem}" ${input} | cut -f4 | sort | uniq`; colnums=`grep "${stem}" ${input} | cut -f5 | sort | uniq`; contrasts=`grep "${stem}" ${input} | cut -f6 | sort | uniq`; merge=`grep "${stem}" ${input} | cut -f7 | sort | uniq`; mergecontrasts=`grep "${stem}" ${input} | cut -f8 | sort | uniq`; genesets=`grep "${stem}" ${input} | cut -f9 | sort | uniq`; label=`grep "${stem}" ${input} | cut -f10 | sort | uniq`; inputgmt=`grep "${stem}" ${input} | cut -f11 | sort | uniq`;rscript=`grep "${stem}" ${input} | cut -f12 | sort | uniq`; rfunctions=`grep "${stem}" ${input} | cut -f13 | sort | uniq`; echo "Rscript ${rscript} --inputanno ${inputanno} --inputexp ${inputcnt}_isoforms-expcnt.tsv --inputtpm ${inputcnt}_genes-tpm.tsv --level \"gene\" --groups ${groups} --colnums ${colnums} --contrasts ${contrasts} --merge ${merge} --mergecontrasts ${mergecontrasts} --genesets ${genesets} --plotlabel ${label} --inputgmt ${inputgmt} --outdir ${outpath_GRADE2071_plots}/ --outstem \"${stem}\" --function ${rfunctions}" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done
