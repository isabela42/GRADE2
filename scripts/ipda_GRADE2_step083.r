...
counts <- counts %>% filter(transcript == outstem)
...
ggsave(file.path(out_boxplot), plot = boxplot, width = 20, height = 20, dpi = 100)

## Beeswarm
beeswarm <- beeswarm_plot(tnorm_counts, plot_condition, outstem)
ggsave(file.path(out_beeswarm), plot = beeswarm, width = 20, height = 20, dpi = 100)
