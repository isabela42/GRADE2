...
Usage: bash ipda_GRADE2_step061-to-pbs.sh -i "path/to/input/files" -p "PBS stem" -e "email" -m INT -c INT -w "HH:MM:SS"
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_GRADE2_step061-to-pbs.sh -i path/to/input/files -p PBS stem -e email -m INT -c INT -w "HH:MM:SS" >&2
           exit;;
    esac
done
...
logfile=logfile_ipda_GRADE2_step061-to-pbs_${thislogdate}.txt
...
outpath_GRADE2061_STAR="GRADE2_step061_alignment_STAR_${thislogdate}"

## Create output directories
mkdir -p ${outpath_GRADE2061_STAR}
...
date
echo "## Executing bash ipda_GRADE2_step061-to-pbs.sh"
...
echo
echo "## Output files saved to:       ${outpath_GRADE2061_STAR}"
...
date ## Status of all user jobs (including GRADE2 step 061 jobs) at
qstat -u "$user"
...
sed -i 's,${outpath_GRADE2061_STAR},'"${outpath_GRADE2061_STAR}"',g' "$logfile"
