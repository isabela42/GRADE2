...
cut -f6 ${input} | sort | uniq | while read stem; do counts=`grep "${stem}" ${input} | cut -f1 | sort | uniq`; metadata=`grep "${stem}" ${input} | cut -f2 | sort | uniq`; thr=`grep "${stem}" ${input} | cut -f3 | sort | uniq`; propTarg=`grep "${stem}" ${input} | cut -f4 | sort | uniq`;  propNTarg=`grep "${stem}" ${input} | cut -f5 | sort | uniq`; echo "awk -v thr=${thr} -v pt=${propTarg} -v pn=${propNTarg} 'NR==FNR{targets[\$1]=1;next} FNR==1{for(i=2;i<=NF;i++){if(\$i in targets){tcol[i]=1;nt++}else{ocol[i]=1;no++}};next} {hit_t=0;hit_o=0;for(i in tcol) if(\$i>=thr) hit_t++; for(i in ocol) if(\$i>=thr) hit_o++; if(hit_t>=pt*nt && hit_o<=pn*no) print \$1}' ${metadata} ${counts} >> ${outpath_GRADE2082_Bash}/${stem}.txt" >> ${pbs_stem}_${stem}_${thislogdate}.pbs ; done
