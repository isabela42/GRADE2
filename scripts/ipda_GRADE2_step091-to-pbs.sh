...
-i <path/to/input/files>    Input TSV files including path FROM working
                            ...
                            Col2:
                            /path/from/working/dir/to/grade051_alignment_STAR_*/salign-star-index_stem/salign-star-index_stemAligned.sortedByCoord.out.bam
                            Or similar BAM file

                            Col3:
                            /path/from/working/dir/to/reference.genome.fasta
                            Note: must have chromosome ids matching the annotation used for generating Col2 file.
...
#-->091 Allele-specific expression stats (1BCFtools, 2ASEP)
...
outpath_GRADE2091_ase="grade091_allele-specific-expression_BCFtools_${thislogdate}"

## Create output directories
mkdir -p ${outpath_GRADE2091_ase}
...
echo
echo "## Output files saved to:       ${outpath_GRADE2091_ase}"
...
cut -f1 ${input} | sort | uniq | while read stem; do echo "#................................................" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read stem; do echo "" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read stem; do echo "module load ${module_bcftools}" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read stem; do echo "" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read stem; do file=`grep "${stem}" ${input} | cut -f2 | sort | uniq`; echo "samtools index -@${ncpus} ${file}" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done
cut -f1 ${input} | sort | uniq | while read stem; do echo "" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done
...
cut -f1 ${input} | sort | uniq | while read stem; do file=`grep "${stem}" ${input} | cut -f2 | sort | uniq`; ref=`grep "${stem}" ${input} | cut -f3 | sort | uniq`; echo "bcftools mpileup --threads ${ncpus} -d0 -Ou -Q 20 -q 20 -f ${ref} ${file} | bcftools call --threads ${ncpus} -m -Ob -a GQ,GP | bcftools query -i 'GT=\"het\"' -f '%CHROM\t%POS\t%REF\t%ALT[\t%GT\t%AD\t%GQ\t%GP]\n' | awk -F'\t' 'BEGIN{OFS=\"\t\"; print \"CHROM\", \"SAMPLE\", \"ID\", \"REF_COUNT\", \"TOTAL_COUNT\", \"GROUP\", \"GQ\", \"P_HOM_REF\", \"P_HET\", \"P_HOM_ALT\"}} {split(\$4, alt, \",\"); split(\$6, ad, \",\"); gq = \$7; split(\$8, gp, \",\"); if (length(alt) == 1 && alt[1] != \"<*>\") {print \$1, \"${stem}\", \$1\":\"\$2, ad[1], ad[1] + ad[2], \$1, gq, gp[1], gp[2], gp[3]}}' >> ${outpath_hippo111_samtools}/${stem}.het.full.counts ; cut -f1-6 ${outpath_hippo111_samtools}/${stem}.het.full.counts > ${outpath_hippo111_samtools}/${stem}.het.4asep.counts" >> ${pbs_stem}_${stem}_${thislogdate}.pbs; done
...
sed -i 's,${module_bcftools},'"${module_bcftools}"',g' "$logfile"
sed -i 's,${outpath_GRADE2091_ase},'"${outpath_GRADE2091_ase}"',g' "$logfile"
